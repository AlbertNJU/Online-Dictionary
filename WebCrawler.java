...
public class WebCrawler {
    public static void main(String[] args) throws IOException {
        ...
        Pattern searchMeanPattern = Pattern.compile("(?s)<div class=\"en-content\">.*?<p>.*?</div>");
        Matcher m1 = searchMeanPattern.matcher(result); //m1是获取包含翻译的整个<div>的

        if (m1.find()) {
            String means = m1.group();//所有解释，包含网页标签
            Pattern getTrans = Pattern.compile("<strong>(.*?)</strong><span>(.*?)</span>");
           // Pattern getPartOfSpeech = Pattern.compile("<strong>(.*?)</strong>");
